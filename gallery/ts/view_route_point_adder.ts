...
class PointAdder {
  ...
  // DOM Elements
  private pairColorInput: HTMLInputElement;
  // Status text will be handled by context.updatePointAdderStatus

  private selectedColor: string = "";
  private selectedPoints: PointToAdd[] = [];
  private canSelectPoints: boolean = false;

  constructor(context: IRouteEditorContext) {
    this.context = context;
    this.svg = this.context.getSvgElement(); // Get SVG from BoardRenderer via context

    this.pairColorInput = document.getElementById(
      "pair-color"
    ) as HTMLInputElement;
    const pairStatusTextElement = document.getElementById(
      "pair-selection-status"
    ); // Used by context

    if (!this.svg) {
      console.error("PointAdder: SVG overlay is missing.");
      this.context.updatePointAdderStatus(
        "Error: Board not initialized for point adding.",
        true
      );
    ...
    if (!this.pairColorInput || !pairStatusTextElement) {
      console.error(
        "PointAdder: Color input or status text element is missing."
      );
      this.context.updatePointAdderStatus(
        "Error: UI elements for point adding are missing.",
        true
      );
      this.pairColorInput.disabled = true;
      return;
  ...
  private addEventListeners(): void {
    this.pairColorInput.addEventListener("input", (e) =>
  ...
  private handleColorChange(e: Event): void {
    ...
    if (this.context.isColorInUseOnBoard(this.selectedColor)) {
      this.canSelectPoints = false;
      this.context.updatePointAdderStatus(
        `Color ${this.selectedColor} is already in use. Delete existing points or choose a different one.`,
        true
      );
    } else {
      this.canSelectPoints = true;
      this.context.updatePointAdderStatus(
        "Now, select two unoccupied points on the grid."
      );
    }
  }
  ...
  private handleSvgClick(e: MouseEvent): void {
    ...
    if (!this.canSelectPoints) {
      if (!this.selectedColor) {
        ...
        this.context.updatePointAdderStatus(
          `Color ${this.selectedColor} is already in use. Choose a different one.`,
          true
        );
      } else if (this.selectedPoints.length >= 2) {
        this.context.updatePointAdderStatus(
          "Two points already selected. Processing...",
          true
        );
      }
      return;
    ...
    if ((e.target as SVGElement).closest("circle.point")) return; // Clicked on existing point

    const cellSize = this.context.getCellSize();
    if (cellSize <= 0) {
      this.context.updatePointAdderStatus(
        "Error: Board not initialized properly (cellSize).",
        true
      );
      return;
    }
    ...
    if (
      ...
      this.context.updatePointAdderStatus(
        "Selected point is outside the board boundaries.",
        true
      );
      return;
    }
    ...
    if (
      ...
      this.context.highlightPointOnBoard(
        x,
        y,
      ...
      this.context.updatePointAdderStatus(
        `Selected ${this.selectedPoints.length} point(s). Select ${
          2 - this.selectedPoints.length
        } more.`
      );
      ...
      this.context.updatePointAdderStatus(
        "Cell is already occupied. Please select an empty cell.",
        true
      );
    } else if (this.selectedPoints.length >= 2) {
      this.context.updatePointAdderStatus(
        "Two points already selected. Please wait or reset.",
        true
      );
    }
  }

  private async processNewPair(
    point1: PointToAdd,
    point2: PointToAdd,
    color: string
  ): Promise<void> {
    ...
    if (!boardId) {
      this.context.updatePointAdderStatus(
        "Error: Could not identify the board.",
        true
      );
    ...
    if (this.context.isColorInUseOnBoard(color)) {
      ...
      this.context.updatePointAdderStatus(
        `Error: Color ${color} is already in use. Operation cancelled.`,
        true
      );
      this.resetSelectionOnError();
      return;
    ...
    if (this.context.isAutoSaveMode()) {
      try {
        const response = await fetch(`/gallery/points/add/${boardId}/`, {
          method: "POST",
          headers: {
        ...
        if (data.success && data.points && data.points.length === 2) {
          this.context.updatePointAdderStatus(
            "Pair auto-saved! Refreshing board..."
          );
          await this.context.fetchBoardDataAndRenderAll(); // Full refresh
          this.resetPairForm();
        } else {
          throw new Error(
            data.error || "Failed to auto-save points or received invalid data."
          );
        }
      } catch (err) {
  ...
  private addPointsManually(
    ...
    const newPoint1Data: PointData = {
      id: tempP1Id,
      x: point1.x,
      y: point1.y,
      color,
    };
    const newPoint2Data: PointData = {
      id: tempP2Id,
      x: point2.x,
      y: point2.y,
      color,
    ...
    this.context.updatePointAdderStatus(
      "Pair added locally. Click 'Save Changes' to persist."
    );
    this.resetPairForm();
  }

  private resetPairForm(): void {
    this.selectedPoints = [];
