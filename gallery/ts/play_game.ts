...
interface GameSessionData {
  ...
  last_updated: string;
  paths: BackendPath[];
  board_details: BoardData;
}

// --- Global Variables & DOM Elements ---
let csrfToken: string = "";
let currentBoardId: string = "";
let game: FreeFlowGame | null = null;

// --- Main Game Class ---
class FreeFlowGame {
  private pathsSvg!: SVGSVGElement;
  private interactionSvg!: SVGSVGElement;
  private boardRenderer!: BoardRenderer;

  private sessionId: number | null = null;
  private boardConfig!: BoardRouteData;
  private points: PointData[] = [];

  private clientPaths: Map<string, ClientPathInfo> = new Map();
  private hasUnsavedChanges: boolean = false;

  private isDrawing: boolean = false;
  private activeDrawingColor: string | null = null;
  private currentDrawingSegments: PathSegment[] = [];
  private tempPathElement: SVGPathElement | null = null;

  private cellSize: number = 30;
  private isSolvedState: boolean = false;

  private saveProgressButton!: HTMLButtonElement;
  private checkSolutionButton!: HTMLButtonElement;
  private resetPathsButton!: HTMLButtonElement;
  private gameStatusElement!: HTMLElement;
  private unsavedIndicator!: HTMLElement;

  private resizeTimeout: number | undefined;
  private drawingCompletedThisStroke: boolean = false;

  constructor(boardId: string, initialCsrfToken: string) {
  ...
  private initializeDOMReferences(): void {
    this.pathsSvg = document.getElementById(
      "paths-svg"
    ) as unknown as SVGSVGElement;
    this.interactionSvg = document.getElementById(
      "interaction-svg"
    ) as unknown as SVGSVGElement;

    this.saveProgressButton = document.getElementById(
      "save-board-button"
    ) as HTMLButtonElement;
    this.checkSolutionButton = document.getElementById(
      "check-solution-button"
    ) as HTMLButtonElement;
    this.resetPathsButton = document.getElementById(
      "reset-all-paths-button"
    ) as HTMLButtonElement;

    this.gameStatusElement = document.getElementById(
      "game-status"
    ) as HTMLElement;
    this.unsavedIndicator = document.getElementById(
    ...
    if (
      ...
      console.error(
        "FreeFlowGame: One or more critical game elements are missing."
      );
  ...
  async initGame(): Promise<void> {
    try {
      const sessionData = await this.apiRequest<GameSessionData>(
        `/gallery/api/game/board/${currentBoardId}/session/`
      );
      ...
      if (boardTitleElement) {
        ...
        if (
          ...
          h1Title.firstChild.textContent = `Playing: ${
            this.boardConfig.name || "Free Flow Game"
          }`;
        }
      }
      ...
      this.boardRenderer.rebuildBoard(
        this.boardConfig.cols,
        this.boardConfig.rows,
        this.points
      );
      ...
      this.updateGameStatus(
        `Error initializing game: ${(error as Error).message}`,
        true
      );
    }
  }
  ...
  private updateGameStatus(
    ...
    let className =
      "mb-4 p-3 text-center rounded transition-all duration-300 ease-in-out ";
    if (isError) type = "error"; // Override type if isError is true for backward compatibility
  ...
  private syncSvgDimensions(): void {
  ...
  private setUnsavedChanges(status: boolean): void {
    this.hasUnsavedChanges = status;
    this.updateButtonStatesAndIndicator();
  }

  private updateButtonStatesAndIndicator(): void {
    if (this.isSolvedState) {
  ...
  private setupEventListeners(): void {
    this.interactionSvg.addEventListener(
      "mousedown",
    ...
    this.saveProgressButton.addEventListener(
      "click",
      this.handleSaveProgress.bind(this)
    );
    this.resetPathsButton.addEventListener(
      "click",
      this.handleResetAllPaths.bind(this)
    );
  ...
  private handleResizeGameElements(): void {
    clearTimeout(this.resizeTimeout);
    this.resizeTimeout = window.setTimeout(() => {
      if (!this.boardConfig || !this.points) return; // Guard against undefined config
      this.boardRenderer.rebuildBoard(
        this.boardConfig.cols,
        this.boardConfig.rows,
        this.points
  ...
  private getCellFromEvent(event: MouseEvent | TouchEvent): PathSegment | null {
    if (!this.boardConfig || !this.interactionSvg) return null;
  ...
  private createTempPathElement(): void {
    if (this.tempPathElement) this.tempPathElement.remove();
    this.tempPathElement = document.createElementNS(
      "http://www.w3.org/2000/svg",
      "path"
    );
    this.tempPathElement.setAttribute("stroke", this.activeDrawingColor!);
    this.tempPathElement.setAttribute(
      "stroke-width",
      (this.cellSize * 0.45).toString()
    );
  ...
  private getPointAt(x: number, y: number): PointData | undefined {
    return this.points.find((p) => p.x === x && p.y === y);
  }

  private getPathOccupyingCell(
    x: number,
    y: number,
    excludeColor?: string
  ): ClientPathInfo | undefined {
  ...
  private removeClientPath(color: string): void {
  ...
  private async handleMouseDown(event: MouseEvent | TouchEvent): Promise<void> {
    if (this.isSolvedState) return;
  ...
  private handleMouseMove(event: MouseEvent | TouchEvent): void {
    ...
    const lastSegment =
      this.currentDrawingSegments[this.currentDrawingSegments.length - 1];
    ...
    if ((dx === 1 && dy === 0) || (dx === 0 && dy === 1)) {
      ...
      const evaluateOption = (
        intermediate: PathSegment,
        final: PathSegment
      ): {
        hitsForeignPoint: boolean;
        deletesColors: Set<string>;
        segments: PathSegment[];
      } => {
        let hitsFP = false;
        ...
        for (const seg of segmentsToEvaluate) {
          ...
          const pathOccupy = this.getPathOccupyingCell(
            seg.x,
            seg.y,
            this.activeDrawingColor!
          );
    ...
    for (const stepCell of proposedSteps) {
      if (this.drawingCompletedThisStroke) break;

      if (
        this.currentDrawingSegments.length > 1 &&
      ...
      if (
        this.currentDrawingSegments.some(
          (s) => s.x === stepCell.x && s.y === stepCell.y
        )
      )
        return;

      const pointInCell = this.getPointAt(stepCell.x, stepCell.y);
      if (pointInCell && pointInCell.color !== this.activeDrawingColor) return;

      const pathToDeleteInfo = this.getPathOccupyingCell(
        stepCell.x,
        stepCell.y,
        this.activeDrawingColor!
      );
      ...
      if (pointInCell && pointInCell.color === this.activeDrawingColor) {
        ...
        if (
          ...
          const colorEndpoints = this.points.filter(
            (p) => p.color === this.activeDrawingColor
          );
          if (colorEndpoints.length === 2) {
            const p1 = colorEndpoints[0];
            const p2 = colorEndpoints[1];
            const currentPathStartPoint = this.getPointAt(
              startSegment.x,
              startSegment.y
            );

            if (
              currentPathStartPoint &&
              pointInCell &&
              currentPathStartPoint.id !== pointInCell.id
            ) {
              if (
                (currentPathStartPoint.id === p1.id &&
  ...
  private async handleMouseUp(event: MouseEvent | TouchEvent): Promise<void> {
    ...
    if (
      this.currentDrawingSegments.length >= 2 &&
      startPointData &&
      endPointData
    ) {
      if (
        startPointData.color === this.activeDrawingColor &&
        ...
        if (colorEndpoints.length === 2) {
          ...
          const p2 = colorEndpoints[1];
          if (
            (startPointData.id === p1.id && endPointData.id === p2.id) ||
  ...
  private updateTempPathDisplay(): void {
    if (
      !this.tempPathElement ||
  ...
  private drawPermanentPath(color: string, segments: PathSegment[]): void {
    if (segments.length < 1 || !this.boardConfig || !this.pathsSvg) return;
  ...
  private removePermanentPathDisplay(color: string): void {
    const pathInfo = this.clientPaths.get(color);
    if (pathInfo?.element) {
      pathInfo.element.remove();
      delete pathInfo.element;
    }
    const existingPathEl = this.pathsSvg.querySelector(
      `path[data-path-color="${color}"]`
    );
    if (existingPathEl) {
  ...
  async handleSaveProgress(): Promise<void> {
    if (!this.sessionId) {
    ...
    const pathsToSave: BackendPath[] = Array.from(
      this.clientPaths.values()
    ).map((pInfo) => ({
      color: pInfo.color,
    ...
    try {
      const response = await this.apiRequest<{
        ...
        `/gallery/api/game/session/${this.sessionId}/save_all_paths/`,
        "POST",
        { paths: pathsToSave }
      );
      ...
      this.updateGameStatus(
        statusMessage,
        false,
        response.is_solved ? "success" : "info"
      );
    } catch (error) {
  ...
  async handleResetAllPaths(): Promise<void> {
    let confirmReset = true;
    if (this.hasUnsavedChanges || this.clientPaths.size > 0) {
      confirmReset = confirm(
        "Are you sure you want to reset all paths? Any unsaved progress will be lost."
      );
    }
    if (!confirmReset) return;

    this.clientPaths.forEach((pInfo) => pInfo.element?.remove());
    if (this.pathsSvg) this.pathsSvg.innerHTML = "";
    this.clientPaths.clear();
  ...
  private checkSolutionLocally(): {
    ...
    const drawnPathColors = new Set<string>(this.clientPaths.keys());
    ...
    for (const [color, pathInfo] of this.clientPaths.entries()) {
      if (!requiredColors.has(color)) continue; // Only check paths for valid, paired colors
      ...
      const pathConnectsP1P2 =
        startCell.x === p1.x &&
        startCell.y === p1.y &&
        endCell.x === p2.x &&
        endCell.y === p2.y;
      const pathConnectsP2P1 =
        startCell.x === p2.x &&
        startCell.y === p2.y &&
        endCell.x === p1.x &&
        endCell.y === p1.y;
  ...
  async handleCheckSolution(): Promise<void> {
    if (this.isSolvedState) {
      this.updateGameStatus(
        "Board is already marked as solved. Save to persist.",
        false,
        "success"
      );
      return;
...
document.addEventListener("DOMContentLoaded", () => {
  const gameDataElement = document.getElementById(
    "game-initial-data"
  ) as HTMLElement;
  const boardId = gameDataElement?.dataset.boardId;
  const csrf = gameDataElement?.dataset.csrfToken;
  ...
  if (boardId && csrf) {
    ...
    console.error(
      "Board ID or CSRF token not found or not replaced in DOM. Game cannot start."
    );
