...
interface GroupedPoints {
  [color: string]: PointData[];
}

class RouteEditorApp implements IRouteEditorContext {
  private boardRenderer: BoardRenderer;
  private settingsManager?: BoardSettingsManager; // Optional if settings panel is not always present
  private pointAdder?: PointAdder; // Optional

  private boardData?: BoardData;
  private currentPointsData: PointData[] = [];
  private pendingChanges: any[] = [];
  private autoSaveModeEnabled: boolean = false;
  private csrfToken: string = "";

  // DOM Elements for RouteEditorApp itself
  private pointsListUL: HTMLElement | null;
  private noPointsMessage: HTMLElement | null;
  private saveChangesButton: HTMLButtonElement | null;
  private routeNameElement: HTMLElement | null;
  private xInputForCoordClick: HTMLInputElement | null; // For SVG coord click
  private yInputForCoordClick: HTMLInputElement | null; // For SVG coord click
  private pairSelectionStatusElement: HTMLElement | null; // For PointAdder status
  ...
  constructor() {
    ...
    this.saveChangesButton = document.getElementById(
      "save-board-button"
    ) as HTMLButtonElement | null;
    this.routeNameElement = document.querySelector("h1.text-3xl");
    this.xInputForCoordClick = document.getElementById(
      "id_x"
    ) as HTMLInputElement | null;
    this.yInputForCoordClick = document.getElementById(
      "id_y"
    ) as HTMLInputElement | null;
    this.pairSelectionStatusElement = document.getElementById(
  ...
  private fetchCsrfToken(): void {
    ...
    if (!this.csrfToken) console.warn("CSRF token not found.");
  }
  ...
  private async init(): Promise<void> {
    ...
    await this.loadInitialBoardData();
  }
  ...
  private addEventListeners(): void {
    ...
    this.boardRenderer.svgElement?.addEventListener("click", (e) =>
      this.handleSvgCoordClick(e)
    );

    // Listen for autoSaveModeChanged events possibly dispatched by settingsManager
    window.addEventListener(
      "autoSaveModeChanged",
      (e: CustomEvent<{ auto_save_enabled: boolean }>) => {
        const newMode = e.detail.auto_save_enabled;
        if (this.autoSaveModeEnabled !== newMode) {
          this.autoSaveModeEnabled = newMode;
          if (this.boardData) this.boardData.route.auto_save_enabled = newMode;

          if (this.autoSaveModeEnabled && this.pendingChanges.length > 0) {
            if (
              confirm(
                "Auto-save is now enabled. You have unsaved changes. Save them now?"
              )
            ) {
              this.savePendingChanges();
            }
    ...
    window.addEventListener("boardDataLoaded", (e: CustomEvent<BoardData>) => {
      ...
      this.settingsManager?.initializePanel(this.boardData); // Inform settings panel
  ...
  public getBoardId = (): string | null =>
    this.boardRenderer.getRouteIdFromUrl() || null;
  public getCsrfToken = (): string => this.csrfToken;
  public getBoardData = (): BoardData | undefined => this.boardData;
  public getCurrentPointsData = (): PointData[] => this.currentPointsData;
  public getPendingChanges = (): any[] => this.pendingChanges;
  public isAutoSaveMode = (): boolean => this.autoSaveModeEnabled;
  public getCellSize = (): number => this.boardRenderer.cellSize;
  public getSvgElement = (): SVGSVGElement | null =>
    this.boardRenderer.svgElement;

  public updateBoardNameInState = (name: string): void => {
    if (this.boardData) this.boardData.route.name = name;
  };
  public updateBoardDimensionsInState = (cols: number, rows: number): void => {
    if (this.boardData) {
      this.boardData.route.cols = cols;
      this.boardData.route.rows = rows;
    }
  };
  public setBoardAutoSaveFlagInState = (enabled: boolean): void => {
    this.autoSaveModeEnabled = enabled;
    if (this.boardData) this.boardData.route.auto_save_enabled = enabled;
  ...
  public findAndModifyPendingChange = (
  ...
  public setCurrentPointsData = (points: PointData[]): void => {
    this.currentPointsData = [...points];
    if (this.boardData) this.boardData.points = this.currentPointsData; // Sync with boardData too
  };

  public removePointsByColorFromState = (color: string): PointData[] => {
    const removed: PointData[] = [];
    ...
    if (this.boardData) this.boardData.points = this.currentPointsData;
    return removed;
  };

  public removePointByIdFromState = (
    pointId: string
  ): PointData | undefined => {
    let removedPoint: PointData | undefined;
    this.currentPointsData = this.currentPointsData.filter((p) => {
  ...
  public updatePageTitle = (name: string): void => {
    if (this.routeNameElement) this.routeNameElement.textContent = name;
  };
  public updateSaveChangesButtonState = (): void => {
    if (this.saveChangesButton) {
      const isDisabled =
        this.autoSaveModeEnabled || this.pendingChanges.length === 0;
  ...
  public fetchBoardDataAndRenderAll = async (): Promise<void> => {
    await this.loadInitialBoardData();
  };
  public highlightPointOnBoard = (
    x: number,
    y: number,
    color: string,
    ...
    Object.assign(marker.style, {
      position: "absolute",
      left: `${
        (x - 1) * this.boardRenderer.cellSize + this.boardRenderer.cellSize / 2
      }px`,
      top: `${
        (y - 1) * this.boardRenderer.cellSize + this.boardRenderer.cellSize / 2
      }px`,
      width: `${radius * 2}px`,
      height: `${radius * 2}px`,
      borderRadius: "50%",
    ...
    if (!isTemporary) setTimeout(() => marker.remove(), 1000); // Auto-remove for generic highlights
  };
  public addPointToSvgGrid = (
  ...
  public removePointFromSvgGrid = (pointId: string): void => {
    const circleToRemove = this.boardRenderer.svgElement?.querySelector(
      `circle.point[data-point-id="${pointId}"]`
    );
    if (circleToRemove) circleToRemove.remove();
  };

  public enablePointDragging = (): void => {
    if (!this.boardRenderer.svgElement) return;
    this.boardRenderer.svgElement
      .querySelectorAll<SVGCircleElement>("circle.point")
      .forEach((circle) => {
        const newCircle = circle.cloneNode(true) as SVGCircleElement;
        circle.parentNode?.replaceChild(newCircle, circle);

        let isDragging = false;
        const originalServerX = parseInt(newCircle.dataset.x ?? "0");
        const originalServerY = parseInt(newCircle.dataset.y ?? "0");
        let dragTargetX = originalServerX;
        let dragTargetY = originalServerY;

        newCircle.addEventListener("mousedown", (e: MouseEvent) => {
          e.preventDefault();
          e.stopPropagation();
          isDragging = true;
          newCircle.classList.add("dragging");
          newCircle.parentNode?.appendChild(newCircle);

          const onMouseMove = (event: MouseEvent) => {
            if (!isDragging || !this.boardRenderer.svgElement) return;
            const rect = this.boardRenderer.svgElement.getBoundingClientRect();
            const potentialX =
              Math.floor(
                (event.clientX - rect.left) / this.boardRenderer.cellSize
              ) + 1;
            const potentialY =
              Math.floor(
                (event.clientY - rect.top) / this.boardRenderer.cellSize
              ) + 1;
            ...
            if (
              ...
              if (
                this.isCellOccupiedOnBoard(potentialX, potentialY) &&
                !(
                  potentialX === parseInt(newCircle.dataset.x!) &&
                  potentialY === parseInt(newCircle.dataset.y!)
                )
              ) {
                const occupier = this.currentPointsData.find(
                  (p) => p.x === potentialX && p.y === potentialY
                );
                if (
                  occupier &&
              ...
              this.boardRenderer.addPointToGrid(
                newCircle,
                dragTargetX,
                dragTargetY
              );
  ...
  public refreshPointsListDisplay = (): void => {
    this.renderPointsListInternal(this.currentPointsData);
  };
  public isCellOccupiedOnBoard = (x: number, y: number): boolean => {
    return this.currentPointsData.some((p) => p.x === x && p.y === y);
  };
  public isColorInUseOnBoard = (color: string): boolean => {
    return this.currentPointsData.some((p) => p.color === color);
  };
  public updatePointAdderStatus = (
    message: string,
  ...
  public addPointsToStateAndPending = (
    ...
    pointsData.forEach((pd) => {
      ...
      if (circle) this.boardRenderer.addPointToGrid(circle, pd.x, pd.y); // Position it
    });
  ...
  // --- Internal Methods ---
  private async loadInitialBoardData(): Promise<void> {
    ...
    await this.boardRenderer.loadAndRenderBoard(routeId);
  }

  private handleSvgCoordClick(e: MouseEvent): void {
    if (this.pointAdder && this.pointAdder.isExpectingInput()) {
      return;
    }

    if (
      !this.xInputForCoordClick ||
    ...
    if (
      targetElement.classList.contains("point") ||
    ...
    const rect = this.boardRenderer.svgElement.getBoundingClientRect();
    const x =
      Math.floor((e.clientX - rect.left) / this.boardRenderer.cellSize) + 1;
    const y =
      Math.floor((e.clientY - rect.top) / this.boardRenderer.cellSize) + 1;
  ...
  private async handlePointUpdate(
    ...
    const pointIndex = this.currentPointsData.findIndex(
    ...
    const originalPointData = { ...this.currentPointsData[pointIndex] };
    ...
    if (this.boardData) this.boardData.points = [...this.currentPointsData]; // Sync boardData
    ...
    if (this.autoSaveModeEnabled) {
      ...
      try {
        const response = await fetch(
          `/gallery/points/update/${this.getBoardId()}/${idStr}/`,
          {
            method: "PUT",
            headers: {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            data.error || `Failed to update point: ${response.status}`
          );
        }
        ...
        this.boardRenderer.addPointToGrid(circleElement, oldX, oldY);
        this.refreshPointsListDisplay();
        alert(`Error auto-saving point: ${(err as Error).message}`);
      }
    } else {
      // Manual save mode
      ...
      if (typeof this.currentPointsData[pointIndex].id === "string") {
        ...
        const tempId = idStr;
        changeModified = this.findAndModifyPendingChange(
          (op) => op.type === "add" && op.temp_ids?.includes(tempId),
          (op) => {
        ...
        // Persisted point
        changeModified = this.findAndModifyPendingChange(
          (op) => op.type === "update" && op.pointId.toString() === idStr,
          (op) => {
            op.x = newX;
  ...
  private renderPointsListInternal(points: PointData[]): void {
    if (!this.pointsListUL || !this.noPointsMessage) return;
    ...
    const grouped: GroupedPoints = points.reduce((acc, point) => {
      acc[point.color] = acc[point.color] || [];
      acc[point.color].push(point);
      acc[point.color].sort((a, b) =>
        ...
        for (let i = 0; i < 2; i++) {
          ...
          if (pointData) {
            ...
            pointDiv.addEventListener("click", (event) => {
              if ((event.target as HTMLElement).closest(".delete-point-btn"))
                return;
              this.highlightPointOnBoard(
                pointData.x,
                pointData.y,
                pointData.color
              );
  ...
  private async handleDeletePointOrPair(
    colorGroupToDelete: string
  ): Promise<void> {
    if (
      !confirm(
        `Are you sure you want to delete the point(s) with color ${colorGroupToDelete}? This will delete the pair.`
      )
    )
      return;

    const pointsInGroup = this.currentPointsData.filter(
      (p) => p.color === colorGroupToDelete
    );
    if (pointsInGroup.length === 0) return;

    const isAnyTemporaryInGroup = pointsInGroup.some(
    ...
    if (this.autoSaveModeEnabled) {
      if (isAnyTemporaryInGroup) {
        ...
        if (typeof idForApiCall !== "number") {
          alert(
            "Error: Cannot delete persisted pair without a valid ID in auto-save."
          );
          return;
        }
        try {
          const response = await fetch(
            `/gallery/api/point/${idForApiCall}/delete/`,
            {
              method: "DELETE",
          ...
          if (!response.ok)
            throw new Error(
              data.error || `Failed to delete point pair ${response.statusText}`
            );
          await this.loadInitialBoardData(); // Refresh all data
        } catch (error) {
        ...
        pointsInGroup.forEach((p) => {
          if (
            ...
            !this.pendingChanges.some(
              (op) => op.type === "delete" && op.pointId === p.id
            )
          ) {
            this.addPendingChange({
              type: "delete",
  ...
  private async savePendingChanges(): Promise<void> {
    ...
    if (!this.saveChangesButton) return;
    ...
    try {
      const response = await fetch(
        `/gallery/api/board/${this.getBoardId()}/save-pending-changes/`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (!response.ok || data.status !== "success") {
        throw new Error(
          data.error || data.message || "Failed to save pending changes."
        );
      }
      ...
      this.rebuildBoardVisuals(
        this.boardData?.route.cols,
        this.boardData?.route.rows,
        this.currentPointsData
      );
      if (this.boardData?.route.name)
        this.updatePageTitle(this.boardData.route.name);
    } catch (error) {
